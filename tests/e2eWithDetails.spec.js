...
test.describe('eCareHealth Complete Appointment Flow Test Suite', () => {
    ...
    test.afterAll(async () => {
        ...
        if (testData.provider) {
            console.log(`👨‍⚕️ Provider Created: ${testData.provider.firstName} ${testData.provider.lastName}`);
            console.log(`   📧 Email: ${testData.provider.email}`);
            console.log(`   🆔 UUID: ${testData.providerUUID}`);
        }
        
        if (testData.patient) {
            console.log(`👤 Patient Created: ${testData.patient.firstName} ${testData.patient.lastName}`);
            console.log(`   🆔 UUID: ${testData.patientUUID}`);
        }
        
        if (testData.appointmentUUID) {
            console.log(`📅 Appointment Booked:`);
            console.log(`   🆔 UUID: ${testData.appointmentUUID}`);
            
            // Calculate and display appointment time
            const today = new Date();
            const nextMonday = new Date();
            const daysUntilMonday = (1 + 7 - today.getDay()) % 7 || 7;
            nextMonday.setDate(today.getDate() + daysUntilMonday);
            ...
            const appointmentDate = nextMonday.toLocaleDateString('en-US', {
                weekday: 'long',
                year: 'numeric',
                month: 'long',
                day: 'numeric'
            ...
            const appointmentTime = nextMonday.toLocaleTimeString('en-US', {
                hour: '2-digit',
                minute: '2-digit',
                timeZoneName: 'short'
            });
